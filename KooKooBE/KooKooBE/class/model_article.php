...
{		   
	...
	*/
    ...
    public function getArticleByStoreID($objID) 
	{		
		$arrArticleID = self::getArticleIDsByStoreID($objID);
		...
		$strSQL = global_common::prepareQuery(global_common::SQL_SELECT_FREE_LIMIT, 
				array($selectField, self::TBL_SL_ARTICLE, $condition.$orderBy,0,self::NUM_PER_PAGE ));					
		//echo $strSQL;
    ...
    /**
	 * Get Aticles By Store ()
	 *
	 * @param mixed $storeIDs This is a description
	 * @return mixed This is the return value description
	 *
	 */
	private function getArticleIDsByStoreID($storeIDs)
	{
		$arrStoreID = global_common::splitString($storeIDs);
		$strQueryIN = global_common::convertToQueryIN($arrStoreID);
		$whereClause = 'WHERE '.global_mapping::StoreID.' IN ('.$strQueryIN.')';
		$strSQL .= global_common::prepareQuery(global_common::SQL_SELECT_FREE,array('*',
					self::TBL_SL_ARTICLE_TYPE_ID,$whereClause));
		//echo $strSQL;
		$articleTypes =  $this->_objConnection->selectCommand($strSQL);	
