...
{		   
	...
	*/
	...
	public function getDistrictByName($districtName,$selectField='*') 
	{		
		$strSQL .= global_common::prepareQuery(global_common::SQL_SELECT_FREE, 
				array($selectField, self::TBL_SL_DISTRICT ,							
	...
	public function getListDistrict($intPage,$orderBy='DistrictID', $whereClause)
	{		
		if($whereClause)
		{
			$whereClause='WHERE'+ $whereClause;						
		...
		for($i=0;$i<$icount;$i++)
		{
			$strHTML.='<tr class="'.($i%2==0?'even':'odd').'">
					...
					<td><input type="checkbox" onclick="_objDistrict.showHide(\''.$arrResult[$i]['DistrictID'].'\',\''.$arrResult[$i]['name'].'\',this)" '.($arrResult[$i]['IsDeleted']?'':'checked=checked').' /></td>
